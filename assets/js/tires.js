$(function () {
        ...
        success: function (r) {
          ...
          if (load_items) {
            ...
            r.data.forEach(function (item, i) {
              html +=
                '<div class="ajax_tires shad__form cnt"><div class="ajax_price">\
								<div class="flex__block"><div>\
								<img src="' +
                (item.img || "/i/tire-icon.png") +
                '" alt="" class="tireImg"><p>' +
                item.code +
                '</p></div><div class="tire_text"><p class="bold">' +
                item.title +
                '</p><p class="bold">' +
                item.price +
                "р.</p><p>Ширина " +
            ...
            $tires.html() ||
              $tires.html(
                '<div class="ajax_tires shad__form cnt">Ничего не найдено. Попробуйте изменить параметры поиска.</div>'
              );
          }
          ...
          if (load_brands) {
            ...
            while (c < col) {
              ...
              for (i = c * q_row; i < (c + 1) * q_row; i++) {
                ...
                item &&
                  (html +=
                    '<label class="brand-item-wrap"><input type="checkbox" class="brand-item" name="brand[]" value="' +
                    item +
                    '" checked> ' +
                    item +
                    "</label>");
  ...
  seasons.forEach(function (item, i) {
    if (!i) return;
    html +=
      '<label><input type="checkbox" class="param-click" name="season[]" value="' +
      i +
      '" checked> ' +
      item +
      "</label>";
  ...
  $(".brand-list").on("click", ".brand-item", function () {
    $(".brand-toggle-all").prop(
      "checked",
      $(".brand-item:checked").length ? false : true
    );
    // toggleItems()
  });
